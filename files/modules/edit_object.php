...
if (isset($_GET['id']))
{
	...
	$current_type_qry = mysqli_query($db,$type_chk_query);

	if ($tp = mysqli_fetch_assoc($current_type_qry))
	{
...
function carefull_delete ($tmp_id,$current_type)
{
	...
	*/
	...
	//echo $count;
	$countRES = mysqli_query($db,$count);

	if ($ct = mysqli_fetch_assoc($countRES))
	{
		if ($ct['total'] > '0')
		{
...
function output_edit_object ($myId, $current_type)
{
	...
	*/
	...
	$theRES = mysqli_query($db,$theSQL);

	if ($in = mysqli_fetch_assoc($theRES))
	{
...
function dropdown_menu($member_of, $indent,$member_of_container)
{
	...
	$conRESULT = mysqli_query($db,$conQRY);

	while ($mo2 = mysqli_fetch_assoc($conRESULT))
	{
